...
export const blockOrder = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const validationError = validateRequiredFields(
      { userId, resourceId, bookingDate, timeSlot, resourceQty },
      null
    );
    ...
    if (!(resourceQty > 0)) {
      return res
        .status(400)
        .json({ error: 'Resource quantity should be greater than 0.' });
    }
    ...
    const blockKey = generateBlockKey(
      userId,
      resourceId,
      bookingDate,
      timeSlot
    ...
    const blockedQty =
      blockedOrders[blockKey]?.reduce(
        (total, block) => total + block.resourceQty,
        0
      ) || 0;

    // Ensure the same user cannot hold more than one block
    if (blockedOrders[blockKey]?.some((block) => block.userId === userId)) {
      const userBlock = blockedOrders[blockKey].find(
        (block) => block.userId === userId
      );
      if (userBlock) {
        const remainingTime = Math.max(
          0,
          userBlock.startTime.getTime() + userBlock.duration - Date.now()
        );
        return res.status(409).json({
          error:
            'User already has a block for the selected resource & time slot.',
          blockStartTime: userBlock.startTime,
          blockEndTime: new Date(
            userBlock.startTime.getTime() + userBlock.duration
          ),
    ...
    const totalBookedQty = existingOrders.reduce(
      (total, order) => total + order.resourceQty,
      0
    );
    ...
    if (totalBookedQty + blockedQty + resourceQty > resource.maxQty) {
      return res.status(400).json({
        error:
          'Not enough availability for the selected time slot.' +
          'alreadyBooked: ' +
          totalBookedQty +
          ', blocked: ' +
          blockedQty +
    ...
    const userBlockCount = Object.values(blockedOrders).reduce(
      (count, blocks) => {
    ...
    if (userBlockCount >= 5) {
      return res
        .status(400)
        .json({ error: 'You cannot block more than 5 resources.' });
    }
    ...
    const timeout = setTimeout(() => {
      blockedOrders[blockKey] = blockedOrders[blockKey].filter(
        (block) => block.userId !== userId
      );
...
export const confirmOrder = async (req: Request, res: Response) => {
  try {
    ...
    if (!blockedOrders[blockKey]) {
      return res
        .status(400)
        .json({ error: 'No block found for the given key.' });
    }

    const blockIndex = blockedOrders[blockKey].findIndex(
      (block) => block.userId === userId
    );
    if (blockIndex === -1) {
      return res
        .status(400)
        .json({ error: 'No block found for the given user.' });
    }
